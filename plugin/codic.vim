...
command! -nargs=? Codic call s:CodicCommand(<f-args>)
...
function! s:CodicCommand(...)
  if a:0 == 0
    call s:CodicSearch(expand('<cword>'))
  else
    call s:CodicSearch(a:1)
  end
endfunction

function! s:CodicSearch(word)
  if len(a:word) == 0
    echohl ErrorMsg
...
function! s:GetDict2(lang)
    if ! exists('s:dict_' . a:lang)
      let dictdir = globpath('<sfile>:p:h', 'dict')
